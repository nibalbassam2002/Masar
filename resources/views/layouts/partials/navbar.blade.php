<nav class="h-16 bg-white border-b border-slate-100 px-8 flex items-center justify-between shrink-0 sticky top-0 z-40">
    ...
    <div class="flex-1 max-w-md">
        <div class="relative group">
            ...
            <input type="text" placeholder="Search tasks or files..."
        ...
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open" @click.away="open = false"
                ...
                <svg :class="open ? 'rotate-180' : ''"
                    class="w-3.5 h-3.5 text-slate-400 transition-transform duration-200" fill="none"
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-200"
                ...
                <a href="#"
                    class="flex items-center gap-3 px-5 py-3 text-xs font-bold text-slate-600 hover:bg-cyan-50 hover:text-cyan-600 transition-all">
                ...
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center gap-3 px-5 py-3 text-xs font-bold text-rose-500 hover:bg-rose-50 transition-all">
                        <svg class="w-4 h-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            stroke-width="2.5">
                            <path
