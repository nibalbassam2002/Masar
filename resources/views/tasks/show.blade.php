    ...
    <div class="max-w-[1400px] mx-auto px-8 py-6">
        ...
        <div class="mb-6 flex items-center">
            <a href="{{ route('tasks.index') }}" class="group flex items-center gap-3 text-slate-400 hover:text-cyan-600 transition-all">
                ...
                <span class="text-[10px] font-black uppercase tracking-widest">Back to Mission Control</span>
            </a>
        </div>
        ...
        @if ($task->parent_id)
            <a href="{{ route('tasks.show', $task->parent_id) }}"
                ...
                <svg class="w-3.5 h-3.5 mr-1.5 transition-transform group-hover:-translate-x-1" fill="none"
            ...
            <div class="lg:col-span-8 space-y-12">
                ...
                <section class="space-y-6">
                    ...
                    <div class="max-h-[400px] overflow-y-auto pr-4 space-y-4 custom-scroll" id="notes-container">
                        ...
                        @empty
                            <p class="text-center py-10 text-xs text-slate-300 italic">No logs posted yet.</p>
                        @endforelse
                    </div>

                    <form action="{{ route('tasks.notes.store', $task->id) }}" method="POST" class="bg-slate-50 p-6 rounded-[2rem] border border-slate-100">
                        @csrf
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="bg-slate-900 text-white px-10 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-cyan-600 transition-all shadow-lg">
                                Post Update
                            </button>
                        </div>
                    </form>
                ...
                <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-5">
                    ...
                    <div class="space-y-3">
                        @foreach ($task->assignees as $assignee)
                            <div class="flex items-center gap-3">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($assignee->name) }}&background=0f172a&color=fff&bold=true" class="w-10 h-10 rounded-xl shadow-sm border border-white">
                                <div>
                                    <p class="text-sm font-bold text-slate-800 leading-none">{{ $assignee->name }}</p>
                                    <p class="text-[9px] font-black text-cyan-600 uppercase mt-1.5">Assigned Member</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                @if ($canManageBreakdown)
                    <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-[10px] font-bold text-slate-300 uppercase">{{ $task->subtasks->count() }} Steps</span>
                        </div>

                        <form action="{{ route('tasks.subtasks.store', $task->id) }}" method="POST" class="space-y-3">
                            @csrf
                            <input type="text" name="title" required placeholder="Delegate a step..." class="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-xs font-medium outline-none focus:ring-1 focus:ring-cyan-500 transition-all">
                            <div class="flex gap-2">
                                ...
                                <button type="submit" class="bg-cyan-500 text-white p-3 rounded-xl hover:bg-cyan-600 shadow-lg shadow-cyan-100">
                        ...
                        <div class="space-y-3 pt-6 border-t border-slate-50">
                            @foreach ($task->subtasks as $sub)
                                <div class="group flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-transparent hover:border-cyan-100 transition-all">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-[9px] font-black text-slate-400 uppercase mt-0.5">{{ $sub->assignees->first()?->name }}</span>
                                        </div>
                                    </div>
                                    <a href="{{ route('tasks.show', $sub->id) }}" class="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-slate-300 hover:text-cyan-600 shadow-sm border border-slate-100 transition-all">
                            ...
                            @foreach ($mySubTasks as $sub)
                                <div class="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10 hover:border-cyan-500/30 transition-all group">
                                    <a href="{{ route('tasks.show', $sub->id) }}" class="flex-1 text-xs font-bold text-white leading-snug hover:text-cyan-400">
                                        {{ $sub->title }}
                                        <span class="block text-[8px] text-slate-500 uppercase mt-1">Open workspace</span>
                                    </a>
                                    <button onclick="updateStatus({{ $sub->id }}, '{{ $sub->status == 'done' ? 'todo' : 'done' }}')"
                                        class="p-2 ml-3 rounded-xl {{ $sub->status == 'done' ? 'bg-emerald-500 text-white' : 'bg-white/10 text-slate-400 group-hover:text-white' }} transition-all">
                ...
                <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                    <form action="{{ route('tasks.notes.store', $task->id) }}" method="POST" enctype="multipart/form-data" id="evidence-upload-form">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <h4 class="text-[10px] font-black uppercase tracking-widest text-slate-400">Shared Evidence</h4>
                    ...
                    <div class="space-y-2">
                        @forelse($task->attachments as $file)
                            <a href="{{ asset('storage/' . $file->file_path) }}" target="_blank" class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl hover:bg-cyan-50 transition-all">
                        ...
                        @empty
                            <div class="py-6 border-2 border-dashed border-slate-50 rounded-2xl text-center opacity-40">
                                <p class="text-[9px] font-bold text-slate-300 uppercase tracking-widest">No Documents Shared</p>
                            </div>
                        @endforelse
                    </div>
                </div>
