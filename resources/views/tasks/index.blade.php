...
@section('content')
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8">

        {{-- Header --}}
        <header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-6 pb-6 border-b border-slate-100">
            <div class="w-full md:w-auto">
                <h1 class="heading-font text-2xl sm:text-3xl font-800 text-slate-900 tracking-tight">
                    {{ auth()->user()->isSuperAdmin() ? 'Global Mission Control' : 'Personal Backlog' }}
                </h1>
                <p class="text-[10px] sm:text-[11px] text-slate-400 font-bold tracking-[0.2em] mt-1 uppercase">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                ...
                <div class="flex bg-slate-100 p-1 rounded-xl w-full sm:w-auto">
                    <a href="{{ route('tasks.index', ['filter' => 'active']) }}"
                        class="flex-1 sm:flex-initial px-4 sm:px-5 py-2 rounded-lg text-xs font-bold transition-all text-center whitespace-nowrap {{ $filter == 'active' ? 'bg-white shadow-sm text-cyan-600' : 'text-slate-500 hover:text-slate-700' }}">
                        Active Tasks
                    </a>
                    <a href="{{ route('tasks.index', ['filter' => 'completed']) }}"
                        class="flex-1 sm:flex-initial px-4 sm:px-5 py-2 rounded-lg text-xs font-bold transition-all text-center whitespace-nowrap {{ $filter == 'completed' ? 'bg-white shadow-sm text-cyan-600' : 'text-slate-500 hover:text-slate-700' }}">
                        Completed
                    </a>
                    <a href="{{ route('tasks.index', ['filter' => 'archived']) }}"
                        class="flex-1 sm:flex-initial px-4 sm:px-5 py-2 rounded-lg text-xs font-bold transition-all text-center whitespace-nowrap {{ $filter == 'archived' ? 'bg-white shadow-sm text-amber-600' : 'text-slate-500 hover:text-slate-700' }}">
                        Archived
                    </a>
                </div>

                {{-- New Task Button --}}
                <a href="{{ route('tasks.create') }}" class="btn-primary inline-flex items-center justify-center gap-2 px-6 py-2.5 w-auto">
